...
import com.lemon.smartcampus.viewModel.character.ScholarViewAction
import com.lemon.smartcampus.viewModel.character.ScholarViewEvent
import com.lemon.smartcampus.viewModel.character.ScholarViewModel
...
fun ScholarListPage(
    ...
    viewModel: ScholarViewModel = viewModel()
) {
    ...
    LaunchedEffect(key1 = Unit) {
        viewModel.dispatch(ScholarViewAction.GetPage(true))
        viewModel.viewEvents.observeEvent(lifecycleOwner) {
            when (it) {
                is ScholarViewEvent.ShowToast -> showToast(
                    it.msg,
